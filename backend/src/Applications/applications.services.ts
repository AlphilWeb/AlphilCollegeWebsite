...
export class ApplicationsService {
    ...
    async getApplicationById(id: number): Promise<InsertApplication | undefined> {
        const application = await db.query.ApplicationsTable.findFirst({where: eq(ApplicationsTable.id , id)})
        return application;
    }
